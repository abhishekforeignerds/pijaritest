            ...
            <div class="card radius-10">
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col-lg-4">
                            <h6 class="card_title">Completed Puja List</h6>
                        </div>
                        <div class="col-lg-8">
                    ...
                    @if (auth()->guard('admin')->user()->canany(['product-edit', 'product-view']))
                        {
                            mRender: function(data, type, row) {
                                ...
                                return '<div class="d-flex order-actions">@if (auth()->guard('admin')->user()->can('product-edit'))<a href="{{ route('e_puja_edit', '') }}/' +
                                    ...
                                    row.id +
                                    '" class="me-2" title="inclusion"><i class="bx bxs-user text-primary"></i></a><a href="{{ route('e_puja_package', '') }}/' +
                                    row.id +
                                    '" class="me-2" title="Show"><i class="bx bxs-show text-warning"></i></a><a href="{{ route('temple_detail_create', '') }}/' +
                                    row.id +
                                    '" class="me-2" title="temple_details"><i class="bx bxs-flag text-danger"></i></a><a href="{{ route('puja_benifit_create', '') }}/' +
                                    row.id +
                                    '" class="me-2" title="Benifits"><i class="bx bxs-shield text-secondary"></i></a><a href="{{ route('one-day-puja.details', '') }}/' +
                                    row.slug +
                                    '" class="me-2" title="E-puja"><i class="bx bx-globe text-danger"></i></a><a href="{{ route('admin.one_day_orders') }}/?puja=' +
                                    row.id +
