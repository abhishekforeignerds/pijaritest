                ...
                <div class="card radius-10">
                    <div class="card-header">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="ms-auto">
                                @if(auth()->guard("admin")->user()->can("category-create"))<a href="{{ route('brand.create') }}"
...
@section('script')
    <script>
        ...
        function update_status(el) {
            ...
            $.post('{{ route('brand.update_status') }}', {
                _token: '{{ csrf_token() }}',
                id: el.value,
                status: status
            }, function(data) {
                ...
                'columns': [
                    ...
                    @if(auth()->guard('admin')->user()->canany(['category-edit','category-view'])) {
                        mRender: function(data, type, row) {
                            return '<div class="d-flex order-actions"> @if(auth()->guard("admin")->user()->can("category-edit"))<a href="{{ route('brand_edit', '') }}/' +
                                row.id + '" class="me-2"><i class="bx bxs-edit"></i></a>@endif  @if(auth()->guard("admin")->user()->can("category-view"))<a href="{{ route('brand_view', '') }}/' +
