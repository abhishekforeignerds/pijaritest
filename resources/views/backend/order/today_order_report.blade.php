    ...
    <div class="page-wrapper">
        <div class="page-content">
            <div class="card radius-10">
                ...
                <div class="card-body">
                    <table>
                        <form method="GET" id="order_form" action="{{ route('admin.today_order_report') }}">
                            <tr>
                                <td>
                                    <div class="mb-3">
                    ...
                    <div class="table-responsive">
                        <table id="example" class="table table-bordered" style="width:99%">
                            ...
                            <tbody id="myTable">
                                ...
                                <tr>
                                    <td class="text-center">{{  optional($order->order_detail)->product_name }}</td>
                                    <td class="text-center">@if(!empty($order->order_detail->product_stock->sku)){{  $order->order_detail->product_stock->sku }}@endif</td>
                                    <td class="text-center">
                                        @if(!empty($dateRange))
                                         {{ App\Models\OrderSubscription::where('product_variant_id',$order->product_variant_id)->where('date',$dateRange)->sum('quantity') }}
                                        @else
                                          {{ App\Models\OrderSubscription::where('product_variant_id',$order->product_variant_id)->sum('quantity') }}
                                        @endif</td>
